...
struct ClientSession::Impl {
    ...
    {
        if (state != State::InWorld || target_net_id == 0) {
            return;
        }

        capnp::MallocMessageBuilder msg;
        auto packet = msg.initRoot<gs::protocol::Packet>();
        auto attack = packet.initAttackTarget();
        attack.setTargetNetId(target_net_id);
        EnqueueFrame(SerializeToFrame(msg));
    }
    ...
    void HandlePayload(const std::vector<std::uint8_t>& payload)
    {
        ...
        try {
            ...
            } else if (packet.isEntityHealthUpdate()) {
                auto update = packet.getEntityHealthUpdate();
                handler.OnEntityHealthUpdate(EntityHealthInfo{update.getNetId(),
                                                              update.getHpCurrent(),
                                                              update.getHpMax()});
            } else if (packet.isEntityDeath()) {
                auto death = packet.getEntityDeath();
                handler.OnEntityDeath(death.getNetId(), death.getKillerNetId());
            } else {
    ...
    void HandleEntitySpawn(gs::protocol::S2cEntitySpawn::Reader response)
    {
        ...
        spawn.mobTypeId = response.getMobType();
        spawn.level = response.getLevel() == 0 ? 1 : response.getLevel();
        spawn.hpCurrent = response.getHpCurrent() <= 0.0f && response.getHpMax() <= 0.0f ? 1.0f : response.getHpCurrent();
        spawn.hpMax = response.getHpMax() <= 0.0f ? std::max(1.0f, spawn.hpCurrent) : response.getHpMax();
...
void ClientSession::SendAttackTarget(std::uint32_t target_net_id)
{
    m_impl->SendAttackTarget(target_net_id);
}
