...
inline WorldCamera BuildFollowCamera(uint32_t width, uint32_t height, WorldVec3 playerPosition, float yawRadians)
{
    const float aspect = height != 0 ? static_cast<float>(width) / static_cast<float>(height) : 1.0f;
    const WorldVec3 forward = {std::sin(yawRadians), 0.0f, std::cos(yawRadians)};
    const WorldVec3 target = WorldAdd(playerPosition, {0.0f, 1.15f, 0.0f});
    const WorldVec3 eye = WorldAdd(WorldSub(target, WorldScale(forward, 18.0f)), {0.0f, 8.0f, 0.0f});
...
inline WorldCamera BuildOrbitCamera(uint32_t width, uint32_t height, WorldVec3 playerPosition,
    float yawRadians, float pitchRadians, float distance)
{
    const float aspect = height != 0 ? static_cast<float>(width) / static_cast<float>(height) : 1.0f;
    const float cosPitch = std::cos(pitchRadians);
    const float sinPitch = std::sin(pitchRadians);
    const WorldVec3 forward = {std::sin(yawRadians) * cosPitch, 0.0f, std::cos(yawRadians) * cosPitch};
    const WorldVec3 target = WorldAdd(playerPosition, {0.0f, 1.15f, 0.0f});
    const WorldVec3 eye = {
